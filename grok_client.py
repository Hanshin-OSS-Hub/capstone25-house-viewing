"""
Grok API 클라이언트 모듈 (xAI, OpenAI 호환 SDK)

역할:
  - 분석 데이터를 Grok에게 전달
  - Grok은 HTML 구조가 아닌 "분석 텍스트 JSON"만 반환
...
_API_KEY: str | None = os.getenv("XAI_API_KEY")
_MODEL_NAME: str = os.getenv("GROK_MODEL", "llama-3.3-70b-versatile")
...
- 위험 점수: {data.ltv_percent}점 (100점 만점)
- 채권최고액 합계: {data.expected_recovery_amount:,}원
...
{{
  ...
  "recovery_comment": "채권최고액 {data.expected_recovery_amount:,}원에 대한 해석과 의미 (2문장)",
...
def generate_analysis_content(data: RiskAnalysisRequest) -> dict:
    """
    Grok API를 호출하여 분석 텍스트 콘텐츠를 생성합니다.
    ...
    Raises:
        ...
        Exception: Grok API 호출 실패 또는 JSON 파싱 실패
    """
    if not _API_KEY:
    ...
    logger.info(f"Grok 분석 텍스트 생성 시작 (model={_MODEL_NAME})")
